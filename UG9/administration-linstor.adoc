...
linstor may select a storage pool on its own. The selection follows these rules:
  ...
  * Discard all nodes and storage pools the current user has no access to
  * Discard all diskless storage pools
  * Discard all storage pools not having enough free space for the autoplace
