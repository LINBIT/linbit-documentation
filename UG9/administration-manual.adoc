[[ch-admin-manual]]
== Common administrative tasks - Manually
...
indexterm:[drbd-overview]`drbd-overview` utility (lines broken here for readability).
...
. `r0` volume `0` is _Primary_ on host 'nina' (the local host), and is mounted as _ext_
  filesystem on `/mnt`.
. Host 'nono' is _Diskless_ for all resources (a <<s-drbd-client,DRBD client>>)
. All other hosts are _UpToDate_ for all resources.
. `r1` is _Secondary_ on all hosts, ie. not used.
. `r2` is used on host 'nini'

`drbd-overview` tries to be exact, but concise; sadly these requirements
conflict with each other, so for more complicated status outputs it might be
easier to read `drbdsetup status` directly.
...
._WFConnection_
indexterm:[connection state, WFConnection]This node is waiting until the peer node
becomes visible on the network.

._WFReportParams_
indexterm:[connection state, WFReportParams]TCP connection has been established, this
...
._StartingSyncS_
indexterm:[connection state, StartingSyncS]Full synchronization, initiated by the
administrator, is just starting. The next possible states are:
_SyncSource_ or _PausedSyncS_.

._StartingSyncT_
indexterm:[connection state, StartingSyncT]Full synchronization, initiated by the
administrator, is just starting. Next state: _WFSyncUUID_.

._WFBitMapS_
indexterm:[connection state, WFBitMapS]Partial synchronization is just
starting. Next possible states: _SyncSource_ or _PausedSyncS_.

._WFBitMapT_
indexterm:[connection state, WFBitMapT]Partial synchronization is just
starting. Next possible state: _WFSyncUUID_.

._WFSyncUUID_
indexterm:[connection state, WFSyncUUID]Synchronization is about to begin. Next
possible states: _SyncTarget_ or _PausedSyncT_.

._SyncSource_
indexterm:[connection state, SyncSource]Synchronization is currently running, with
the local node being the source of synchronization.

._SyncTarget_
indexterm:[connection state, SyncTarget]Synchronization is currently running, with
the local node being the target of synchronization.

._PausedSyncS_
indexterm:[connection state, PausedSyncS]The local node is the source of an ongoing
...
._PausedSyncT_
indexterm:[connection state, PausedSyncT]The local node is the target of an ongoing
synchronization, but synchronization is currently paused. This may be
due to a dependency on the completion of another synchronization
process, or due to synchronization having been manually interrupted by
`drbdadm pause-sync`.
...
._VerifyS_
indexterm:[connection state, VerifyS]On-line device verification is currently
running, with the local node being the source of verification.

._VerifyT_
indexterm:[connection state, VerifyT]On-line device verification is currently
...
Lastly perform an `apt-get update` so Debian recognizes the updated repository.
...
Or, even better yet - start using <<ch-admin-drbdmanage,DRBD Manage>>, which 
will provide an easy way to "take over" your active resources at some time in 
the future, to make administration even easier!
...
Now, the only thing left to do is to get the DRBD devices up and running again 
- a simple `drbdadm up all` should do the trick.
...
_online_ againfootnote:[At least that's the state at the time of writing 
- that's how it has been in the past, and we want to keep it that easy. But who 
knows? Who can tell? ;)].
...
WARNING: In DRBD 9.0.x Dual-Primary mode is not supported. It might work "by 
accident", but don't rely on it. Because of that, changes in the configuration
are very likely -- for example, with three Primaries the setting 
`allow-two-primaries` is incorrect already... +
In the 9.1 series multiple Primaries will be possible and supportedfootnote:[See also the 
roadmap: http://drbd.linbit.com/home/roadmap/].
...
to <<re-drbdconf#after-sb-0pri>> for details on split brain recovery keywords not
discussed here.
...
The quorum implementation in DRBR gets enabled by setting the `quorum` resource
