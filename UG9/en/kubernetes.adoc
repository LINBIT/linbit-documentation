...
* Configure the LINSTOR database backend. By default, the chart configures Etcd as database backend. Starting with
  1.7.0, the operator can also configure LINSTOR to use <<s-kubernetes-linstor-k8s-backend,Kubernetes as datastore>>
...
Starting with LINSTOR 1.16.0, the Controller can use the Kubernetes API directly to persist its cluster state. To enable
...
csi:
  enabled: true
  pluginImage: "drbd.io/linstor-csi:v0.17.0"
...
operator:
  ...
  image: "drbd.io/linstor-operator:v1.7.0"
  affinity: {} <4>
  tolerations: [] <4>
  resources: {} <3>
  podsecuritycontext: {}
  controller:
    enabled: true
    controllerImage: "drbd.io/linstor-controller:v1.17.0"
    dbConnectionURL: ""
    luksSecret: ""
    dbCertSecret: ""
  ...
  satelliteSet:
    enabled: true
    satelliteImage: "drbd.io/linstor-satellite:v1.17.0"
    storagePools: {}
    sslSecret: ""
    ...
    tolerations: [] <4>
    resources: {} <3>
    monitoringImage: "drbd.io/drbd-reactor:v0.5.1"
    kernelModuleInjectionImage: "drbd.io/drbd9-rhel7:v9.1.4"
    kernelModuleInjectionMode: ShippedModules
    kernelModuleInjectionResources: {} <3>
    additionalEnv: [] <5>
haController:
  enabled: true
  image: drbd.io/linstor-k8s-ha-controller:v0.2.1
  affinity: {} <4>
  tolerations: [] <4>
  resources: {} <3>
  replicas: 1 <2>
...
with Kubernetes, pass `--set stork.enabled=false` to the install command.
