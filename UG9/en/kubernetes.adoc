...
Starting with Linstor Operator v1.5.0, you can use [Prometheus](https://prometheus.io/) to monitor Linstor components.
The operator will set up monitoring containers along the existing components and make them available as a `Service`.

If you use the [Prometheus Operator](https://prometheus-operator.dev/), the Linstor Operator will also set up the `ServiceMonitor`
