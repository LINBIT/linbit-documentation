...
indexterm:[Proxmox]This chapter describes DRBD in Proxmox Virtual Environment (VE) via
...
nodes. The preferred way to use the plug-in, starting from version 4.1.0, is via LINSTOR resource groups and a
...
Starting from version 5 of the plug-in one can set the option "preferlocal yes". If it is set, the plug-in tries
