...
----
--x-replicas-on-different <auxiliary-property-name> <integer>
----

The integer value is the maximum number of resource replicas that LINSTOR can place on nodes that have the same value for the specified auxiliary property.
...
Other combinations of `--x-replicas-on-different` and `--place-count` integer values are possible.
