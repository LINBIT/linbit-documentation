...
Since `linstor-server` version 1.5.2 and a recent LINSTOR client, you can use LINSTOR to create
LVM volumes or ZFS pools on a satellite node. The LINSTOR client has a `physical-storage`
command and various sub-commands to list possible disks and create LINSTOR storage pools from
these disks. You can then make these LINSTOR storages pools available for use with LINSTOR
storage objects, such as LINSTOR resource groups, definitions, and resources, for your
applications and services to consume.

However, although you can use LINSTOR to create these LVM or ZFS backed storage pools, LINSTOR
does not have access to LVM or ZFS management commands. Only the `pvcreate`, `vgcreate`, and
`zpool create` commands are available to LINSTOR. LINSTOR cannot use the `pvremove`, `vgremove`,
or `zpool destroy`, commands, for example. You will have to perform management and deletion
actions manually on the nodes, by using appropriate LVM or ZFS commands.

[[s-physical-storage-listing]]
===== Listing Physical Storage for LINSTOR Use

The following command will show you a table that lists the available disks grouped by size and
type (rotational). The rotational attribute will be 'True' if the device is a spinning, magnetic
hard disk drive, and 'False' (empty entry in the table list shown) if this device is an SSD,
NVMe, or other "non-rotational" drive.
...
[IMPORTANT]
====
The `physical-storage list` command will only show disks that pass the following filters:
...
[[s-physical-storage-creating]]
===== Creating Physical Storage for LINSTOR Use

With the `physical-storage create-device-pool` command you can create a LVM or ZFS pool on a
disk and also directly add it as a storage pool in LINSTOR. For example, to create an LVM thin
provisioned pool the uses `/dev/vdc` as a backing device, you can enter the following command:
...
tool) starts too many shipments at once, leading to an overload of the network or
some of the nodes sending the backups.
...
The expected value for this property is a number. Setting it to any negative number will be interpreted
as "no limit", while setting it to zero will result in this specific node not being eligible to
ship any backups - or completely disabling backup shipping if the property is set to 0 on the controller.

Any other positive number is treated as a limit of concurrently active shippings per node.
To determine which node will send a backup shipment, LINSTOR uses the following logic in the order shown:

1. The node specified in the command (`--source-node` for shippings to another cluster, `--node` for
   shipping to S3 compatible storage) will ship the backup.
2. The node that has the most available backup slots will ship the backup.
3. If no node has an available backup slot, the shipment will be added to a queue and started as soon as
   a different shipment has finished which leads to a backup slot becoming available.
