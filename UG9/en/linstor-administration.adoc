...
For configuring the highly available storage you can use LINSTOR itself. This has the advantage that the storage is
under LINSTOR control and can for example be easily extended to new cluster nodes. Just create a new
resource with 200MiB in size. Given and existing storage pool named `pool_ssd`, you can create a new LINSTOR resource named `linstor_db`, by entering the following commands:

----
# linstor resource-group create my_ssd_group --storage-pool pool_ssd --place-count 3
# linstor resource-group drbd-options --on-no-quorum=io-error my_ssd_group
# linstor resource-group drbd-options --auto-promote=no my_ssd_group
# linstor volume-group create my_ssd_group
# linstor resource-group spawn-resources my_ssd_group linstor_db 200M
----
