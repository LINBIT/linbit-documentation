...
[[s-linstor-setupopts]]
=== Setting DRBD Options on LINSTOR Objects

You can use LINSTOR commands to set DRBD options. Configurations in files such as `/etc/drbd.d/global_common.conf` that are not managed by LINSTOR will be ignored. You can enter the LINSTOR object along with `drbd-options` and the `--help` or `-h` flag to show the command usage, available options, and the default value for each option. Some examples are:
...
An exception to this is the LINSTOR resource object. With the LINSTOR resource object, you use the `drbd-peer-options` to set DRBD options at the connection level between the two nodes that you specify.

----
# linstor resource drbd-peer-options -h
----

For example, to set the DRBD protocol for a resource named `backups`, enter:
...
To remove a DRBD option, prefix the option name with the word `unset`. For example:

----
# linstor resource-definition drbd-options --unset-protocol backups
----

Removing a DRBD option will return the option to its default value.
