...
Any other positive number is treated as a limit of concurrently active shippings per node. To
determine which node will send a backup shipment, LINSTOR uses the following logic in the order
shown:

1. The node specified in the command (`--source-node` for shippings to another cluster, `--node`
...
properties `SnapshotShipping/TargetNode` as well as `SnapshotShipping/RunEvery` on the
