...
* The storage back-end driver to use for the storage pool on the cluster node (LVM, ZFS, 
and others)
...
To access the images, you first have to login to the
...
3. Add `luks` to the layer-list. Note that all plug-ins (e.g., Proxmox) require a DRBD layer as the top most layer if they do not explicitly state otherwise.
...
1. Stop the running linstor-controller using systemd: `systemctl stop linstor-controller`
