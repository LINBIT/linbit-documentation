...
===== Definitions
Definitions define attributes of an object, and can be thought of as
...
NOTE: If you have a support subscription to LINBIT(R), you will have access to
...
You can place different volumes of a resource in different storage pools by setting the
`StorPoolName` property on the volume definitions before the resource is deployed to the nodes:
...
NOTE: Because the `volume-definition create` commands are used without the `--vlmnr` option,
LINSTOR automatically assigns the volume numbers starting at 0. In the preceding example, the 0
and 1 in the two `volume-definition set-property` commands reference these automatically
assigned volume numbers.

Here the 'resource create' commands do not need a `--storage-pool` option.
If you do not specify a storage pool, LINSTOR will use a 'fallback' storage pool. To determine
that storage pool, LINSTOR queries the properties of the following objects,
...
falls back to a hard-coded 'DfltStorPool' string to use as a storage pool.
...
While the above methods can re-route DRBD traffic, and LINSTOR communication between the
controller and satellite nodes, it is not possible through `linstor` commands only, to route
LINSTOR controller-client traffic (commands that you issue from a LINSTOR client to the
controller) through a specific NIC. To achieve this, you can either:

. Specify a LINSTOR controller using methods outlined in <<s-using_the_linstor_client>> *and*
