...
If you are a LINBIT(R) customer, you can download a LINBIT created helper script and run it on your
nodes to:
...
IMPORTANT: In both cases (hash + distribution, and bind-mounting a repository)
the hash or repository configuration has to be from a node that has a special property set. Feel
...
When you use `linstor node list` LINSTOR will show that the new node
is marked as offline. Now start and enable the LINSTOR satellite service on the new node
...
becoming online. Of course the satellite process might be started before
...
LINSTOR storage pool name might have different VG or zPool names on the
...
to hot swap storage devices, you might choose a model where you create
...
Using resource groups to define how you want your resources
...
With the `resource create` command you can assign a resource definition
...
`BalanceResourcesInterval`:: The interval in seconds, at the LINSTOR controller level, that the balance resource placement task is triggered. By default, the interval is 3600 seconds (one hour).

`BalanceResourcesGracePeriod`:: The period in seconds for how long new resources (after being created or spawned) are ignored for balancing. By default, the grace period is 3600 seconds (one hour).
...
LINSTOR can select a storage pool on its own when you create (spawn) resources from the resource group. The selection follows these rules:
...
satellite nodes. Next, you create a resource group, `testRscGrp`, with a placement count of two and
...
This tool has an executable file called `/usr/share/linstor-server/bin/linstor-database`. This executable
has two subcommands, `export-db` and `import-db`. Both subcommands accept an optional `--config-directory`
argument that you can use to specify the directory containing the `linstor.toml` configuration file.

IMPORTANT: To ensure a consistent database backup, take the controller offline by stopping the controller
...
For example, to restore the previously made backup from the `db_export` file, enter the following
commands:
...
specify the correct `linstor.toml` configuration file by using the `--config-directory` argument
(or updating the default `/etc/linstor/linstor.toml` and specifying the database type that you want
to use before importing). See the <<#s-linstor-external-database,LINSTOR User's Guide>> for more
...
configured your integration plugins (for example, the Proxmox plugin) to be ready for
...
file system without an existing storage-device. The data of the
...
NOTE: The option `--diskless` is deprecated. Use the `--drbd-diskless`
or `--nvme-initiator` commands instead.
...
`--layer-list`. However, note that the `Writecache/PoolName` refers to
...
Refer to `linstor controller set-property --help` for a list of `Cache/*`
...
For a list of providers with their properties refer to <<s-storage-proviers, Storage Providers>>.
...
* File / FileThin: Mostly used for demonstration or experiments. LINSTOR will reserve a
...
* Exos [DEPRECATED]: This special storage provider is currently required to be run on a "special satellite".
Refer to the <<ch-exos, EXOS Integration>> chapter.
...
Refer to <<s-remote-spdk, Remote SPDK Provider>> for more details.
...
the controller is the leftmost, that is, the least prioritized level).
...
quorum features and their behavior, refer to the
...
You might want to disable DRBD's quorum features completely. To do that,
...
configured network interfaces, storage pools, properties and similar entities,
...
might have. This could be the case, for example, if you have specified number of replicas for
...
which can be used to complete nearly every object that LINSTOR knows about,
for example, node names, IP addresses, resource names, and others.
The following examples show some possible completions, and their results:
...
==== From the Community
For help from the community, subscribe to our mailing list located here: https://lists.linbit.com/listinfo/drbd-user

==== GitHub
To file bug or feature request, check out our GitHub page https://github.com/linbit
