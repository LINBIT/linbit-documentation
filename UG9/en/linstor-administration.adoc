...
On SLES, DRBD is normally installed through the software installation component of YaST2. It comes bundled with the High Availability
package selection.
...
CentOS has had DRBD 8 since release 5. For DRBD 9 you will need look to EPEL and similar sources.
Alternatively, if you have an active support contract with LINBIT you can use our RHEL 8 repositories.
DRBD can be installed using `yum`. We can also check for the newest version of the LVM-tools as well.

IMPORTANT: LINSTOR *requires* DRBD 9 if you want to have replicated storage. This requires an external
...
By default a LINSTOR cluster consists of exactly one LINSTOR controller (a node running the LINSTOR Controller software). Making LINSTOR highly available
involves providing replicated storage for the LINSTOR Controller database, multiple LINSTOR controllers where only one
is active at a time, and a service manager that takes care of mounting and unmounting the highly available storage as well as starting and stopping the LINSTOR Controller (`linstor-controller`) services.

==== Configuring Highly Available Storage

For configuring the highly available storage we use LINSTOR itself. This has the advantage that
the storage is under LINSTOR control and can for example be easily extended to new cluster
nodes. Just create a new resource with 200MB in size. First, create a resource group (named
`rg_ha_ctrl`) based on an existing storage pool (named `pool1`) and placed across three nodes.


----
# linstor resource-group create rg_ha_ctrl --storage-pool pool1 --place-count 3
----


Then create a volume group from it.

----
# linstor volume-group create rg_ha_ctrl
----

Next, use the `resource-group spawn-resources` command to have LINSTOR automatically create a
new resource that uses the resource group that you just created as a template, with a resource
definition name (`linstor_db`), and size (200M) that you specify.

----
# linstor resource-group spawn-resources rg_ha_ctrl linstor_db 200M
----

From now on we assume the resource's name is `linstor_db`. It is crucial that your cluster
qualifies for auto-quorum and uses the `io-error` policy (see Section
<<s-linstor-auto-quorum>>), and that `auto-promote` is disabled.

Set these DRBD options by entering the following commands:
...
You can verify the resource definition's DRBD options by using LINSTOR's `resource-definition
list-properties` command.


After the resource is created, it is time to move the LINSTOR database to the new storage and to create a `systemd`
mount service. First stop the current `linstor-controller` service and disable it, because later you will configure https://linbit.com/drbd-user-guide/drbd-guide-9_0-en/#ch-drbd-reactor[DRBD Reactor] to manage it.
...
which you want to become a possible LINSTOR controller node. It is important that the controllers
...
installing packages on your operating system: SLES: `zypper`; CentOS: `yum`.
...
. Specify a LINSTOR controller using methods outlined in <<s-using_the_linstor_client>> *and* have the only route to the controller as specified be through the NIC you would like to use for controller-client traffic.
