...
commands. This section will use the `resource create --auto-place` command in examples.
...
Maybe not so obvious is that you can omit the `--storage-pool` option, then
LINSTOR can select a storage pool on its own. The selection follows these rules:
...
As OpenFlex combines concepts of LINSTOR's storage and NVMe-layer, LINSTOR was
added both, a new storage driver for the storage pools and a dedicated `openflex`
...
When using NVMe, LINSTOR was designed to run on both sides, the NVMe target and on the
...
Refer to <<s-openflex, OpenFlex(TM) Layer>> for more details.
