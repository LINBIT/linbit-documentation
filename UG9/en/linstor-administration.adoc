...
to find the linstor-controller. As shown, this can be specified as a command line option, an
...
Alternatively you can create the `/etc/linstor/linstor-client.conf` file and populate it like
below.
...
NOTE: The LINSTOR client commands can also be used in a much faster and convenient way by only
writing the starting letters of the parameters. For example, rather than entering `linstor node
list` you can enter the LINSTOR short notation command `linstor n l`.
...
====
You can create or modify a resource group and specify a placement count or other contraint that
