...
For example, to cancel the `linstor resource toggle-disk --diskless` command shown earlier, if it became stuck, enter a `linstor resource toggle-disk --storage-pool pool_ssd` command.
If a diskless resource becomes stuck when trying to toggle it to a diskful state, you would enter a `linstor resource toggle-disk --diskless` command to cancel the operation.
