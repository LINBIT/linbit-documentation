...
2.6.26, so running it on older systems (e.g. RHEL 5) is not possible; here
...
* _DRBD-conn_: The first few packets are being pushed across the connection; but still e.g. a Split-Brain situation might serve it again.
...
NOTE: While the commands above are only accepted from UID 0 (i.e., the `root` user),
...
The currently available plugins are  `zstd`, `lz4`, `zlib` and
