...
case is a file system opener that does not do any IO and just idles around. Such processes need to be removed
...
keyword value `force-secondary`. This enables DRBD to automatically recover a primary node that
loses quorum with suspended I/O operations. With this option configured, when such a node
connects to a cluster partition that has a more recent data set, DRBD automatically demotes the
primary node that lost quorum and has suspended I/O operations. Additional configurations, for
example in a `handlers` section of the resource configuration file, as well as additional
configurations within a cluster manager, may also be necessary to complete a fully automatic
recovery setup.
