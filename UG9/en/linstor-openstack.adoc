...
<6> *Deprecated in 2.0.0, use <<s-openstack-volume-types,volume types>> instead.* The storage pools to use when placing resources. Applies to all diskfull resources created. Defaults to `DfltStorPool`.
...
If this is the only error message, this means the Cinder Scheduler could not find a volume
