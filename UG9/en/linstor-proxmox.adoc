...
https://www.proxmox.com/en/[Proxmox VE] is an easy to use, complete server
...
Proxmox VE. This repository not only contains the LINSTOR Proxmox plugin, but the entire LINBIT SDS stack,
...
TIP: The Proxmox version is the Debian (Proxmox base OS) version minus 4.

==== Installing the LINSTOR Proxmox plugin and related utilities from packages

After configuring an appropriate LINBIT package repository, you can install the LINSTOR Proxmox plugin and
...
.LINSTOR Proxmox plugin storage configuration options
|===
| Option | Meaning
...
Before upgrading the plugin, upgrade the core LINSTOR packages on all nodes as described
in <<linstor-administration.adoc#s-upgrading,Upgrading LINSTOR>>.
Proxmox VE nodes typically run both the LINSTOR controller and satellite services, so
follow the combined role instructions in that section.

If you need to do a fresh installation, skip this section and continue with
<<s-proxmox-ls-install>>.

[[s-proxmox-ls-upgrades-plug-x-8]]
==== Upgrading to plugin version 8.x
...
==== Upgrading to plugin version 7.x
...
==== Upgrading plugin version from 4.x to 5.x
...
IMPORTANT: Storage entries within the Proxmox storage configuration file *must* be separated by empty new lines.

[[s-proxmox-ls-upgrades-plug-5-6]]
==== Upgrading plugin version from 5.x to 6.x
...
==== Upgrading PVE from version 5.x to 6.x
