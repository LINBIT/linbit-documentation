...
ifndef::drbd-only[]
If you are a LINBIT customer you can enable LINBIT's `drbd-9` repository on your node. You can then install the LINSTOR Proxmox plug-in by using a package manager.


NOTE: Refer to the Using a Script to Manage LINBIT Cluster Nodes for instructions on registering a node with LINBIT and enabling LINBIT repositories.
