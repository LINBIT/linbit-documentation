...
* If you used the <<s-kubernetes-creating-operator-v2-kustomize, `kustomize` route>>, apply the upgrade by changing the
  resource link in your `kustomization.yaml` file.
+
.kustomization.yaml
[source,yaml]
----
...
* If you used the <<s-kubernetes-creating-operator-helm, Helm route>>, apply the upgrade by upgrading the `linstor-operator`
  Helm chart.
+
----
$ helm repo update
...
Version v2.3 removed the `NetworkPolicy` resource from default deployment. To clean up existing `NetworkPolicy`
...
Version v2.2 removed the dependency on cert-manager for the initial deployment. To clean up existing `Certificate`
