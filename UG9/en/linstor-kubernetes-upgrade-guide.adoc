...
If you installed the LINSTOR Affinity Controller separately by using Helm, it is no longer required.
The Operator now deploys the LINSTOR Affinity Controller as part of LINBIT SDS.
...
Version 1.9 disables the <<s-kubernetes-ha-controller-v1,LINSTOR HA Controller>> deployment by
...
<<s-kubernetes-snapshots-working-with>> for instructions on how you can add the snapshot-controller to your cluster.
