...
NVMe-oF targets, iSCSI targets, and NFS exports by using both LINSTOR(R) and DRBD Reactor
behind-the-scenes.
...
node can be diskless, to save on costs, and only needs to serve as a DRBD quorum witness. For
...
For NVMe-oF, iSCSI, and NFS, a LINSTOR storage pool, resource group`, and volume group for
LINSTOR Gateway need to be created before use. For more detail regarding LINSTOR storage pools,
refer to the <<linstor-administration.adoc#s-storage_pools>> section of the _LINSTOR User's
Guide_.
...
LINSTOR Gateway requires that you change the LINSTOR satellite configuration on each satellite
node. To do this, edit or create the `/etc/linstor/linstor_satellite.toml` configuration file
...
DRBD Reactor is a daemon that will orchestrate the iSCSI, NFS or NVMe-oF resources in the
LINSTOR Gateway cluster. You must install DRBD Reactor and enable the DRBD Reactor service on
all LINSTOR satellite nodes in the cluster.

Each DRBD Reactor daemon on a LINSTOR satellite node tries to take over DRBD Reactor managed
services. Managed services include DRBD resources that maintain identical, up-to-date copies of
data on every diskful node. The node running the daemon that wins the race prevents other nodes
from activating the services. If the current node running managed services is no longer able to,
for example, due to a loss of network connection, a system reboot, or other such failure event
then a new race for managing the services starts and a new winner will take over. In this way,
DRBD Reactor makes the resources and services that it manages highly available.
...
Instructions might vary for an installation from link:https://github.com/LINBIT/drbd-reactor[the
DRBD Reactor GitHub project]. Refer to the `README` file and other documentation in the GitHub
...
DRBD Reactor to automatically reload when its configuration changes. To do this, enter the
following commands on all DRBD Reactor nodes:
...
and configure various packages and utilities. This is described in the subsections that follow. 

===== Installing NVMe-oF Utilities

If you will use LINSTOR Gateway to create and manage NVMe-oF targets, you will need to install
...
RHEL distribution's repositories, is needed for installing DKMS. Development tools and other
dependencies for building SCST's RPMs must also be installed:
...
kernel module, create a systemd unit file for an `iscs-scst` service, and reload systemd
services:
...
Reactor's ability to manage the service. Disable the `nfs-server` service and verify that it has
been disabled using the following commands:
