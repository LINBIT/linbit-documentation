...
`drbdmon` utility. It updates the state of DRBD resources in real time.
...
[[s-drbdsetup-events2]]
==== One-shot or real time monitoring via `drbdsetup events2`
...
--
  ...
  * _no_: No congestion.
  * _upper_: I/O _above_ the DRBD device is blocked, i.e. to the filesystem. Typical causes are
...
mode>> can be switched to the primary role on two nodes; this is e.g. needed for
...
(e.g. because files got deleted while one node wasn't connected), you
...
perhaps _resize_ (e.g. for a virtual machine), you should be fine.
====================
...
Please note that removing the resource from a currently _active_ node (i.e.
...
therefore keeps quorum, while the secondary becomes outdated. Note that the secondary's
