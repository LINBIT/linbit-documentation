...
`on-congestion` optione (see <<s-configure-congestion-policy>>).
...
(_Primary_/_Secondary_) manually; only stopping of the services and umounting,
...
number of nodes that can communicate is greater then the half of the overall number
...
it has to assume that all away nodes form a patition and have access to up-to-date
