...
*_C~s~_* is the data device size in sectors, and *_N_* is the maximum number of
peer bitmap slots configured in the metadata (`bm-max-peers`).

NOTE: These formulas assume the default bitmap granularity of 4 KiB per bit.
If you use a different `bm-bytes-per-bit` setting, the bitmap portion of the
metadata scales inversely with the granularity.
...
.Historical UUIDs
These are the identifiers of preceding data generations, maintained
as a rolling history. New entries are pushed to the front, and older
entries are shifted out when the buffer is full.
...
In the quick-sync bitmap, one bit represents a chunk of on-disk data.
By default, each bit covers 4 KiB, but since DRBD 9.3, this is configurable up to 1 MiB
per bit. If the bit is cleared, it means that the corresponding block is
...
https://clusterlabs.org/doc/crm_fencing.html.] the peer
