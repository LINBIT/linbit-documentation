...
The quorum mechanism built into DRBD and enabled by LINSTOR prevents any split-brain to happen.
...
attempts of starting the KVM process on another hypervistor host, by failing the `open()` system call.
