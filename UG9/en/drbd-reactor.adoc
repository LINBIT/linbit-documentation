...
[[s-drbd-reactor-promoter-plug-in-configuring-multiple-start-list-items]]
==== Configuring Multiple Start List Items and Items Spanning Multiple Lines

A promoter plug-in snippet file can have a start list that contains multiple start items. You
can also specify start list items that span multiple lines, perhaps for clarity reasons. An
example start list that shows both of these cases is:

----
start = [
"foo.service",
"""
ocf:vendor:resource_agent1 instance_ID
key1=value
key2=value
key3=value
""",
"""
ocf:vendor:resource_agent2 instance_ID
key1=value
key2=value
key3=value
""",
...
ifndef::de-brand[]
You can install the `resource-agents` package provided in LINBIT's customer repositories to
install a suite of resource agents.
endif::de-brand[]
...
because your want the plug-in to react to changes. This is only possible if two states, old
...
- A DRBD configured resource named _ha-mount_ that is backed by a DRBD device, `/dev/drbd1000`,
  on all nodes
+
IMPORTANT: Change the DRBD device's minor number (`1000`) in the configuration files and
commands that follow if yours is different.
...
ifndef::de-brand[]
+
If you already installed the `resource-agents` package provided in LINBIT's customer repositories, then the Filesystem OCF resource should already be on your system.
endif::de-brand[]
...
start = [
"""
ocf:heartbeat:Filesystem fs_test
device=/dev/drbd1000
directory=/mnt/test
fstype=ext4
run_fsck=no
...
In the example configuration above, `fs_test` is the optional instance ID name that you give the
`Filesystem` resource agent. If you omit the name, The `drbd-reactorctl status` command will
show an instance ID name based on the `device` parameter specified in the TOML configuration
file.
...
`scrape_config` section. Replace “node-x” in the `targets` lines below with either hostnames or
