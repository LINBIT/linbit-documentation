...
DRBD Reactor uses systemd behind the scenes to react to DRBD events.
DRBD Reactor can generate and use systemd units to start or stop services on nodes in a cluster, for example, to mount a file system, to start a database service, to run Open Cluster Framework (OCF) resource agents, and others.
...
You can install DRBD Reactor from source files found within https://github.com/LINBIT/drbd-reactor[the DRBD Reactor source code repository].
See the instructions there for details and any prerequisites.

ifndef::drbd-only[]
Alternatively, LINBIT customers can install DRBD Reactor from officially supported prebuilt packages, available from the LINBIT `drbd-9` packages repository.
endif::drbd-only[]

DRBD Reactor packages are also available in link:https://launchpad.net/~linbit/+archive/ubuntu/linbit-drbd9-stack[the LINBIT PPA] for the latest two LTS Ubuntu releases.

WARNING: Although the LINBIT PPA can be a great option for a homelab, testing, or other non-production use, LINBIT does not offer any guarantees, support, or SLAs for the packages in this public repository.
For production or enterprise deployments, using the official LINBIT customer repositories is recommended.
...
Besides systemd services such as these examples, the promoter plugin supports link:https://github.com/ClusterLabs/resource-agents[OCF resource agents].
For example, you can configure the promoter plugin to start an IPaddr2 resource agent which can create a virtual IP address on a node.
This ensures that you can reach services through a consistent IP address, no matter which node in your cluster is running them.
The promoter plugin also supports failure actions such as rebooting a node if a resource fails to demote, so that the resource can promote on another node.

The `drbd-reactor.promoter` manual page describes in detail the mechanisms and logic that the promoter plugin uses behind the scenes to start or block services from running on nodes.
...
The full set of metrics and an example Grafana dashboard are available at the https://github.com/LINBIT/drbd-reactor/blob/master/doc/prometheus.md[DRBD Reactor source code repository].
...
Global configurations are made within a main TOML configuration file, `/etc/drbd-reactor.toml`.
The file has to be a valid TOML (https://toml.io) file.
Plugin configurations should be made within TOML snippet files that can be placed into the default DRBD Reactor snippets directory, `/etc/drbd-reactor.d`, or into another directory if specified in the main configuration file.

For documentation purposes only, the example configuration file also shows example plugin configurations.
...
preferred-nodes-policy = always <8>
----
...
Actions can be one of: `none`, `reboot`, `reboot-force`, `reboot-immediate`, `poweroff`, `poweroff-force`, `poweroff-immediate`, `exit`, `exit-force`.
...
<8> Since DRBD Reactor v1.9.0, you can set a preferred nodes policy of `always` (the default if unspecified) or `start-only`.
By setting a `start-only` preferred nodes policy, promoter plugin configured services will not fail back to a more preferred node if they are already running on a lower priority node.
Avoiding "automatic switchback" behavior might be useful if you prefer minimizing service disruption to always trying to run services on particular nodes.

See the `drbd-reactor.promoter` manual page for more background and information about the promoter plugin configuration settings.
...
See the https://github.com/LINBIT/drbd-reactor/blob/master/doc/umh.md[UMH documentation page] in the DRBD Reactor source code repository for more details, explanations, examples, and caveats.
...
- Remove a specified DRBD Reactor plugin and reload the DRBD Reactor daemon, by using the `drbd-reactorctl rm` command.

- List the activated plugins, or optionally list disabled plugins, by using the `drbd-reactorctl ls [--disabled]` command.

- Start a subset of services in a promoter plugin start list, by using the `drbd-reactorctl start-until` command.
...
[[s-drbd-reactorctl-status]]
==== Showing DRBD Reactor status

By using the `drbd-reactorctl status` command, you can show the status of DRBD Reactor plugins that are enabled on the node.
You can specify the name of a plugin snippet file (with or without the `.toml` file extension) to show only its status.
You can also specify more than one plugin snippet file to show the status of each.
If you do not specify a plugin snippet file name, the command shows the status of all enabled plugins on a node.

The following example output from a `drbd-reactorctl status` command shows the status of two enabled plugins, a promoter and a Prometheus plugin, on a node:
...
● ├─ ocf.rs@fs_2_nfs.service
● ├─ ocf.rs@nfsserver_nfs.service
● ├─ ocf.rs@export_drbd_nfs_vol1_nfs.service
● ├─ ocf.rs@export_drbd_nfs_vol2_nfs.service
● ├─ ocf.rs@virtual_ip_nfs.service
● └─ ocf.rs@portunblock_nfs.service
/etc/drbd-reactor.d/prom.toml:
Prometheus: listening on :9942
----

In this output, a green filled circle shows that a promoter plugin configured service unit is running on the node.
An empty circle shows that a service is configured but not running on the node, as shown in the following example output from running the same `drbd-reactorctl status` command on another node.

----
/etc/drbd-reactor.d/nfs.toml:
Promoter: Currently active on node 'node-0'
○ drbd-services@nfs.target
○ ├─ drbd-promote@nfs.service
○ ├─ ocf.rs@portblock_nfs.service
○ ├─ ocf.rs@fs_cluster_private_nfs.service
...
The `drbd-reactorctl status` indicators for a promoter plugin service unit state are:

- Active: a bold green filled circle (●)
- Reloading: a bold green refresh arrow (↻)
- Inactive: a plain circle (○)
- Failed: a bold red cross (×)
- Activating: a bold filled circle (●)
- Deactivating: a bold filled circle (●)
- Maintenance: a plain circle (○)
...
- The Cluster Labs OCF Filesystem resource agent, available through https://github.com/ClusterLabs/resource-agents/blob/main/heartbeat/Filesystem[the Cluster Lab `resource-agents` source code repository], should be present in the `/usr/lib/ocf/resource.d/heartbeat` directory
+
ifndef::de-brand[]
NOTE: If you have a LINBIT customer or evaluation account, you can install the `resource-agents` package available in the LINBIT `drbd-9` package repository to install a suite of open source resource agent scripts, including the OCF Filesystem resource agent.
endif::de-brand[]
...
You can find a list of the available metrics in https://github.com/LINBIT/drbd-reactor/blob/master/doc/prometheus.md[the documentation folder] in the project's source code repository.
