...
- Support for container networking. If you don't have the https://www.cni.dev/[Container Network
  Interface] plugins installed, you will only be able to use `mode = "host"` in your job
  networks. For most production setups, we recommend installing the default plugins:
+
...
<5> In order to configure storage devices, DRBD and load kernel modules, the containers need to
...
<9> The `drbd-loader` container can be configured via environment variables. `LB_HOW` tells the
...
<8> The `csi_plugin` stanza informs Nomad that this task is a CSI plugin. The Nomad agent will
...
<2> The `name` is used when creating the volume in the backend (i.e. LINSTOR). Ideally this
