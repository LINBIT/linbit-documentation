...
first create a `kustomization` YAML configuration file, by entering the following command:
...
a different version of the monitoring configuration file. You can do this by entering the
following command:
...
deployment by enabling monitoring in the `linbit-sds` chart. Enter the following command to do
this:
...
NOTE: Unlike the `kustomize` deployment method, you can use the same `helm install` command for
...
image::images/linstor-k8s-prometheus-alertmanager-drbdconnectionnotconnected.png[alerts for the
`my-res` DRBD resource in a disconnected state]
