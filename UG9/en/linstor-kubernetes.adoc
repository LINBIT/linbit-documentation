...
Users of existing Operator v1 deployments should continue to use their Helm deployments,
<<s-kubernetes-deploy-linstor-operator-v1,skipping to the Operator v1 instructions>>.

[[s-kubernetes-deploy-linstor-operator-v2]]
==== Deploying LINSTOR Operator v2

LINSTOR Operator v2 is deployed using
link:https://kubernetes.io/docs/tasks/manage-kubernetes-objects/kustomization[`kustomize` tool],
integrated with `kubectl`.

[[s-kubernetes-deploy-linstor-operator-v2-prerequisites]]
===== Prerequisites for LINSTOR Operator v2

To deploy LINSTOR Operator v2, you need to meet the following prerequisites:

* link:https://cert-manager.io/docs/installation/[Install `cert-manager`]. You can quickly install `cert-manager` by
applying its static manifests:
+
----
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/latest/download/cert-manager.yaml
----
* link:https://kubernetes.io/docs/tasks/tools/[Install `kubectl` version `>= 1.22`].
* Have your link:https://my.linbit.com[my.linbit.com] credentials ready.
...
Make sure to replace `MY_LINBIT_USER` and `MY_LINBIT_PASSWORD` with your own credentials.
...
resources:
  - https://github.com/LINBIT/linstor-operator-builder//deploy/default?ref=v2.1.1&depth=1&timeout=90s <1>
generatorOptions:
...
----
<1> Depending on your internet connection, you can adjust the timeout value higher if you are
having issues pulling the LINSTOR Operator when you apply the YAML file. For example, if you get
an "accumulation err" error message about exceeding a timeout when accumulating resources from
the `linbit-operator-builder` GitHub repository.
...
Deploying LINBIT SDS for Kubernetes with the Operator is a simple as creating a new `LinstorCluster` resource and
...
Other types of storage pools can be configured as well. Please check
...
<<s-kubernetes-basic-configuration-and-deployment,guide for creating basic volumes>>, or refer to the
available link:https://github.com/piraeusdatastore/piraeus-operator/tree/v2/docs/tutorial[tutorials] and
link:https://github.com/piraeusdatastore/piraeus-operator/tree/v2/docs/how-to[how-to guides] upstream.
...
<<s-kubernetes-deploy-linstor-operator-v2, Operator v2>>.
...
using this operator. The following guides assume the operator is installed <<s-kubernetes-deploy-linstor-operator-v1,using Helm>>
...
pricing or to begin a trial]. Alternatively, you may use LINSTOR SDS' upstream project named
link:https://github.com/piraeusdatastore/piraeus-operator[Piraeus], without being a LINBIT
customer.
...
<10> If set to true, the satellite containers will have the following files and directories mounted from the host OS:
+
...
[[s-kubernetes-monitoring-dashboards-and-alert-rules]]
==== Using Prometheus Metrics to Create a Monitoring Dashboard and Alert Rules

One of the useful things that you can do with the cluster-wide metrics that the LINSTOR controller exports and that Prometheus collects is to create monitoring dashboards and alert rules. This can make it easier to digest the metrics and also automate alerts when events occur that might need user intervention.

[[s-kubernetes-grafana-dashboard-installing]]
===== Installing a Grafana Monitoring Dashboard

To get started you can install a custom-built LINSTOR and DRBD Grafana dashboard, available at https://grafana.com/grafana/dashboards/14339-drbd/[grafana.com]. This dashboard was built by Deckhouse.io and is freely available.

To install the dashboard, navigate to the dashboards page of your Grafana instance, click on the "New" button, and then click on "Import". You can then import the Deckhouse.io built dashboard by referencing its ID (15917) that you can find on the linked page above, and then clicking on the "Load" button. On the next "Import Dashboard" screen, select a Prometheus data source, "Prometheus (default)" under the "main" option, and then click the "Import" button.

The Grafana instance will then load the "LINSTOR / DRBD" dashboard.

[[s-kubernetes-prometheus-alertrules-creating]]
==== Creating Prometheus Alert Rules

You can create alert rules based on Prometheus scraped cluster metrics by editing and then applying a YAML file or files to your Kubernetes deployment.



[[s-kubernetes-grafana-configuring]]
==== Configuring a Grafana Dashboard and Alerts
...
After verifying that your cluster is running all right, you can continue to follow the steps in
