...
is necessary that Pacemaker's communication stays available when DRBD's network
link breaks.

The quorum mechanism, however, takes a completely different approach. The basic
...
data set is not touched, so that the node does not create a diverging data set.
