...
NOTE: In DRBD 9, `/proc/drbd` only shows version information. Per-resource status
output has been removed. Use <<s-drbdadm-status>> or <<s-drbdsetup-events2>> instead.

indexterm:[/proc/drbd]`/proc/drbd` is a virtual file that was used
extensively in DRBD 8.4 to display resource status. In DRBD 9, it only
shows the DRBD module version and build information:

----
$ cat /proc/drbd
version: 9.x.y (api:2/proto:118-124)
GIT-hash: XXX
----


[[s-drbdadm-status]]
==== Retrieving status information by using the DRBD administration tool
...
other IDs as well. This line shows the indexterm:[replication state]
replication state in the
`replication` item (see <<s-replication-states>> for details) and the
...
[[s-drbdsetup-events2]]
==== Retrieving status by information using the DRBD setup command
...
and written to. This role only occurs on one node, unless
...
neither be read from nor written to. This role can occur on more than one
nodes.
...
That is, DRBD has sent it to the peer and is waiting for the acknowledgment that it has been written.
...
and is waiting for the acknowledgment that it has been written.
In sectors (512 bytes).
...
Rate of Synchronization] section of this user guide.
...
_VerifyT_ as replication state, meaning that the `verify` is still in progress.
...
might occasionally become congested. In a default configuration, this
would cause I/O on the primary node to block, which is sometimes
undesirable.

If and only if you are using DRBD with <<s-using-drbd-proxy,DRBD Proxy>>, you can configure DRBD to _suspend_ the ongoing replication
in this case, causing the data set on the Primary node to _pull ahead_ of data sets on Secondary nodes. In this suspended mode, DRBD keeps the replication channel open -- it
...
When using DRBD Proxy with DRBD to replicate over a WAN, you can set both `congestion-fill` and
`congestion-extents` together with the `pull-ahead` option, if you might need to try to avoid DRBD disconnections or other symptoms related to DRBD replication over a sometimes congested WAN link.
...
[[s-three-nodes]]
=== Creating a stacked 3-node setup
...
For flexibility, you can also set the `quorum` option to a numerical value. This quorum
...
WARNING: If you choose to set the `quorum` property to a numerical value, choose a numerical
