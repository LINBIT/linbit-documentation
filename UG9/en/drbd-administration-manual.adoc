...
during the next attach attempt, since the block device would now be too small (if
external metadata was in use), or it would be unable to find its metadata
(if internal metadata was in use because DRBD metadata is written to the end of the backing block device). To work around these
...
+
You must dump the metadata on all nodes that are configured for the DRBD resource, by using a separate dump file for each
node.
+
IMPORTANT: *Do not* dump the metadata on one node and then simply copy the dump
file to peer nodes. *This. Will. Not. Work.*
...
* Reimport the corrected metadata, on both nodes:
+
----
...
Consider the following example for a five node cluster. It requires a partitions to have at
least three nodes, and two of them must be `UpToDate`:
...
Ideally the application simply terminates in case of I/O errors. This then allows Pacemaker
...
node marked its data as Outdated, and that it was able to tell the remaining nodes
that its data is Outdated.

In a group of nodes where all disks are Outdated no one in that group can promote to Primary
footnote:[The exception is a manual promotion with the `--force` flag. It's assumed that
before you use the `--force` flag, you know what you are doing.].
