...
external metadata is used), or it would be unable to find its meta
data (in case internal metadata is used). To work around these
...
You must do this on both nodes, using a separate dump file for every
node. *Do not* dump the metadata on one node, and simply copy the dump
