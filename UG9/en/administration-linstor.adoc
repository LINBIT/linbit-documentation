...
Using the `--auto-place` option, it is possible to have LINSTOR select nodes or storage pools
to assign a resource definition to.
The value after the `--auto-place` option tells LINSTOR how many replicas you want to have.
...
NOTE: If everything was successful, the DRBD resource has now been created by LINSTOR.
...
* LVM / LVM-Thin: The adminstrator is expected to specify the LVM volume group or the thin-pool
 (in form of "LV/thinpool") in order to use the corresponding storage type. These drivers
...
* `linstor backup delete all myRemote`: This command deletes ALL S3-objects on the given remote,
 as long as they are recognized to be backups, that is, fit the expected naming schema.  There
...
More information on how to install and configure etcd can be found here:
