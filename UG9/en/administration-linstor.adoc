...
INFO: Since a remote needs to store sensitive data, such as passwords, it is
...
INFO: Usually, LINSTOR uses the endpoint and bucket to create an url using the virtual-hosted-style
...
INFO: All `linstor backup delete ...` commands have the `--dry-run` option, which will
