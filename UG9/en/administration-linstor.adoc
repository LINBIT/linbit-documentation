...
===== Node
Nodes are servers or containers that participate in a LINSTOR cluster. The _Node_
attribute defines:
...
profiles or templates. Objects created will inherit the configuration
defined in the definitions. A definition must be defined before creating
the associated object.  For example; you must create a _ResourceDefinition_
before creating the _Resource_
...
As we download DRBD's newest module we can check if the LVM-tools are up to date as well. Users who prefer command-line install
may simply issue the following command to get the newest DRBD and LVM versions:
...
repository to be configured, either LINBIT's or a 3rd party.
...
then it is recommended to report the error to LINBIT and restore the old database file and downgrade to your previous
controller version.

If you use any external database or etcd, it is recommended to do a manual backup of your current database to have
a restore point.

So first upgrade the `linstor-controller`, `linstor-client` package on your controller host and restart the `linstor-controller`,
...
==== Storage pools shared by multiple nodes
...
pool to use the same LVM2 volume group accessible by 2 or more nodes. The example below shows using the LVM2
...
all requirements. This step also considers other auto-placement restrictions
such as `--replicas-on-same`, `--replicas-on-different` and others.
...
are manged by `drbd-reactor`, so make sure the `linstor-controller.service` is
...
running from the previous step. Also make sure to set `chattr +i /var/lib/linstor` on all potential controller
...
Check that there are no warnings from drbd-reactor in the logs by running `systemctl status drbd-reactor`.
...
In order to use NVMe-TCP instead of the default NVMe-oF the following
...
two providers there are already sub-types for their thinly provisioned variants.
...
(in form of "LV/thinpool") in order to use the corresponding storage type. These drivers support
...
file in a given directory and will configure a https://man7.org/linux/man-pages/man4/loop.4.html[loop device]
on top of that file.
...
* SPDK: The administrator is expected to speicify the logical volume store which LINSTOR should use.
The usage of this storage provider implies the usage of the <<s-nvme-layer,NVME Layer>>.
...
Administrators might not want to save the `--api-pw` in plaintext, which would be clearly visible
...
neccessary to have encryption enabled whenever you wish to use a remote in any way.
...
for its access to the given bucket (e.g. my-bucket.s3.us-west-2.amazonaws.com). Should your setup not
allow access this way, change the remote to path-style access (e.g. s3.us-west-2.amazonaws.com/my-bucket)
...
* `linstor backup delete all myRemote`: This command deletes ALL S3-objects on the given
remote, as long as they are recognized to be backups, that is, fit the expected naming schema.
There is the option `--cluster` to only delete backups that were created by the current cluster.
...
* `linstor backup delete s3key myRemote randomPictureInWrongBucket`: This command will find
the object with the given S3-key and delete it - without considering anything else. This should
...
Backups can be downloaded from any cluster, not just the one that uploaded them, as long as the setup
...
is issued). On the target side, you only need a linstor-remote with the cluster-id of the source-cluster set.
If you wish to ship a snapshot inside the same cluster just use a remote pointing to the local controller.
...
When the snapshot you wish to ship contains a LUKS layer, the remote on the target cluster also needs
...
The controller has a configuration file that is and has to be placed into the folowing path: `/etc/linstor/linstor.toml`.
...
The satellite has an optional configuration file that uses the TOML file syntax and has to be put into the following path `/etc/linstor/linstor_satellite.toml`.
