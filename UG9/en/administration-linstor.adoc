...
# linstor resource-definition set-property linstor_db DrbdOptions/Resource/on-no-quorum io-error
...
cluster qualifies for auto-quorum and uses the `io-error` policy (see Section <<s-linstor-auto-quorum>>).
...
component on all nodes that could become a LINSTOR controller and edit their `/etc/drbd-reactor.d/linstor.toml` configuration
...
`drbd-reactor`. Do not edit the service files directly, but use `systemctl edit`. Edit
