...
IMPORTANT: LVM scans all devices if an LVM-command is used. If your LINSTOR-cluster
scales up this causes a big performance loss. Therefore you should change and uncomment
the following line in the /etc/lvm/lvm.conf file to prevent LVM from doing this on DRBD devices:
----------------------------
# global_filter = [ "a|.*|", "r|/dev/drbd.*|" ]
----------------------------
...
LINSTOR can be used without DRBD as well. Without DRBD it has the only ability to create local
...
Alternatively, if you wish to purchase remote installation services, 24/7 support, access to certified repositories, or feature development please contact sales@linbit.com or call 1-877-454-6248 (1-877-4LINBIT)
