...
`drbd-utils` supports DRBD 9 kernel modules. Legacy support for DRBD 8.4 can optionally be
included at build time (`--with-84support`).
...
"Vanilla" kernel tar files from the https://kernel.org/ archive are simply named
...
https://forums.linbit.com[LINBIT Community Forum], it is usually wise to
either attach a copy of that file or point others to a location from
...
The DRBD kernel module source always targets the newest upstream kernel.
Backward compatibility with older kernels is handled automatically by a
https://github.com/coccinelle/coccinelle[Coccinelle]-based compat system
that generates the necessary patches at build time.

[[s-build-from-tarball]]
===== Building from a release tarball

Release tar files ship pre-generated compat patches for common distribution
kernels. In most cases, building is straightforward:
...
By default, this builds the module for the currently running kernel
(using `/lib/modules/$(uname -r)/build`). To build against a different
kernel, set `KDIR`:

----
$ make clean
$ make KDIR=/path/to/kernel/headers/or/source
----

For kernels without pre-generated compat patches, the build automatically
uses the LINBIT https://github.com/LINBIT/saas/["spatch as a service"] web service to generate them. If you do not
have internet access, you will need a local Coccinelle installation.

[[s-build-from-git]]
===== Building from a Git-cloned repository

Building from a Git-cloned repository requires Coccinelle (`spatch`) for the compat system.
Refer to the https://github.com/LINBIT/drbd/blob/master/README.md[DRBD README]
...
If the module build completes successfully, you can find the built
...
After you find the newly-built kernel module file, you can show information about it by entering:

----
$ /sbin/modinfo <path/to/kernel/module>
----
...
install DRBD by issuing the commands:

----
$ cd drbd && sudo make install && cd ..
